...
class DeleteCeleb(Task):
    ...
    def run(self):
        ...
        sscd_model = SSCD_Embedder(accelerator.device)

        # Precompute deletion embeddings ONCE
        print("Computing deletion embeddings (SSCD)...")
        deletion_loader = torch.utils.data.DataLoader(dataset_deletion, batch_size=32)

        deletion_feats = []
        for x in deletion_loader:
            x = x.to(accelerator.device)
            deletion_feats.append(sscd_model(x).detach())

        deletion_feats = torch.cat(deletion_feats)  # shape: [N_del, D]
        ...
        while global_step < self.cfg.training_steps * len(self.cfg.deletion.img_name):
            ...
            # ===== SSCD-FILTERED SYNTHETIC SAMPLING =====

            filtered_synthetic = []

            while len(filtered_synthetic) < self.cfg.train_batch_size:

                with torch.no_grad():
                    candidates = synthetic_pipeline(
                        batch_size=self.cfg.synthetic_pool_size,
                        num_inference_steps=50
                    ).images

                candidates = torch.stack([
                    transform(img) for img in candidates
                ]).to(device=accelerator.device, dtype=weight_dtype)

                # Compute embeddings
                cand_feats = sscd_model(candidates)

                # Similarity with deletion set
                sim = cand_feats @ deletion_feats.T
                max_sim = sim.max(dim=1).values

                # FILTER CONDITION
                keep_mask = max_sim < self.cfg.sscd_threshold

                selected = candidates[keep_mask]

                if selected.shape[0] > 0:
                    filtered_synthetic.append(selected)

            # Final synthetic batch
            synthetic_images = torch.cat(filtered_synthetic, dim=0)[:self.cfg.train_batch_size]
            synthetic_images = synthetic_images.detach()
            
            # ===== RANDOM SAMPLING =====

            #change <0.5 to adjust sampling
            mask = torch.rand(self.cfg.train_batch_size, device=real_images.device) < 0.3
